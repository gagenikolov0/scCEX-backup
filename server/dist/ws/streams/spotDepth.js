...
async function send(symbol, depth) {
    try {
        ...
        const bids = Array.isArray(j?.bids) ? j.bids.slice(0, depth).map((x) => [Number(x[0] ?? x.price ?? x[1]), Number(x[1] ?? x.qty ?? x[2])]) : [];
        const asks = Array.isArray(j?.asks) ? j.asks.slice(0, depth).map((x) => [Number(x[0] ?? x.price ?? x[1]), Number(x[1] ?? x.qty ?? x[2])]) : [];
...
//# sourceMappingURL=spotDepth.js.map
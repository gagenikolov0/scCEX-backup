...
exports.moveMoney = moveMoney;
const SpotPosition_1 = __importDefault(require("../models/SpotPosition"));
async function moveMoney(session, userId, asset, amount, action) {
    const numAmount = Number(amount);
    if (isNaN(numAmount))
        throw new Error("Invalid amount: " + amount);
    let inc = {};
    if (action === 'SPEND') {
        const pos = await SpotPosition_1.default.findOne({ userId, asset }).session(session);
     ...
     */
    const updated = await SpotPosition_1.default.findOneAndUpdate({ userId, asset }, { $inc: inc, $set: { updatedAt: new Date() } }, { session, upsert: true, new: true });
...
//# sourceMappingURL=moneyMovement.js.map
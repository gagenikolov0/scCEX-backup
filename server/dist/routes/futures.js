    ...
    try {
        ...
        await session.withTransaction(async () => {
            ...
            if (futAcc.available < 0.0000000001)
                futAcc.available = 0; // If balance is infinitesimally small, snap to zero
            if (type === 'limit')
                ...
                if (position) {
                    if (position.side === side) {
                        ...
                        if (baseQuantity >= position.quantity) {
                            ...
                            const futAcc = await FuturesAccount_1.FuturesAccount.findOne({ userId, asset: quoteAsset }).session(session);
                            if (futAcc) {
        ...
        (0, emitters_1.syncFuturesBalances)(userId).catch(() => { });
        (0, emitters_1.syncFuturesPosition)(userId, symbol).catch(() => { });
        if (orderDoc)
...
router.delete('/orders/:id', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const orderId = req.params.id;
        (0, emitters_1.syncFuturesBalances)(userId).catch(() => { });
        (0, emitters_1.syncFuturesPosition)(userId, canceledSymbol).catch(() => { });

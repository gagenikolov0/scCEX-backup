...
const cache = new Map();
async function fromCache(key, ttlMs, fetcher) {
    const now = Date.now();
...
router.get('/spot/24h', async (req, res) => {
    try {
...
router.get('/spot/klines', async (req, res) => {
    try {
        const { symbol, interval = '1m', limit = '2000' } = req.query;
        if (!symbol)
            return res.status(400).json({ error: 'symbol is required' });
        const key = `spot:klines:${symbol}:${interval}:${limit}`;
        const ttl = interval === '1m' ? 2000 : interval === '5m' ? 10000 : interval === '1h' ? 60000 : 300000;
        const data = await fromCache(key, ttl, async () => {
            const candidates = [interval];
            if (interval === '1h')
                candidates.push('60m');
            if (interval === '2h')
                candidates.push('120m');
            if (interval === '4h')
                candidates.push('240m');
            if (interval === '6h')
                candidates.push('360m');
            if (interval === '1w')
                candidates.push('7d');
            if (interval === '2d')
                candidates.push('2880m');
            let lastErr = null;
...
router.get('/spot/intervals', async (req, res) => {
    try {
        const symbol = String(req.query.symbol || '');
        if (!symbol)
            return res.status(400).json({ error: 'symbol is required' });
        ...
        const data = await fromCache(key, 5 * 60 * 1000, async () => {
            ...
            return checkResults.filter((res) => res !== null);
        });
...
// Futures: klines
router.get('/futures/klines', async (req, res) => {
    try {
        const symbol = String(req.query.symbol || '');
...
router.get('/futures/intervals', async (req, res) => {
    try {
        ...
        const data = await fromCache(key, 5 * 60 * 1000, async () => {
            ...
            let results = checkResults.filter((res) => res !== null);
...
router.get('/futures/24h', async (req, res) => {
    try {
        const symbol = String(req.query.symbol || '');
        ...
        const data = await fromCache(key, 2000, async () => {
            ...
            if (sym) {
                ...
                return arr.find((r) => r?.symbol === sym) || {};
            }
            return j;
        });
